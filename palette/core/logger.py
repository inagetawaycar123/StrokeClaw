...
class VisualWriter:
    ...
    def save_images(self, results):
        ...
        try:
            ...
            if names[0].endswith(
                ".npy"
            ):  # if到else都是新加的内容，else后面到except是原来的内容
                outputs = [image.numpy().squeeze() for image in results["result"]]
                for i in range(len(names)):
                    # output_gray = ((outputs[i]*0.186+0.157)*255).astype('uint8')  # logger 未来可能有问题，现在只有Tmax格式的输出反归一化，未来需要修改 mean=0.157, std=0.186
                    np.save(os.path.join(result_path, names[i]), outputs[i])
                    # universal_show_img(outputs[2], "gray")
                    # x = 1
                    # output_gray = (outputs[i]*255).astype('uint8')
                    # if output_gray.ndim == 3:
                    #     np.save(os.path.join(result_path, names[i]), output_gray)
                    # else:
                    #     gray_img = Image.fromarray(output_gray, mode='L')
                    #     np.save(os.path.join(result_path, names[i]), output_gray)
                    #     loc = os.path.join(result_path, names[i].replace('.npy', '.png'))
                    #     gray_img.save(loc)
            else:
                outputs = Util.postprocess(results["result"])
                for i in range(len(names)):
                    Image.fromarray(outputs[i]).save(
                        os.path.join(result_path, names[i])
                    )
