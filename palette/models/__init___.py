...
def define_network(logger, opt, network_opt):
    ...
    net = init_obj(
        network_opt, logger, default_file_name="models.network", init_type="Network"
    )

    if opt["phase"] == "train":
        logger.info(
            "Network [{}] weights initialize using [{:s}] method.".format(
                net.__class__.__name__, network_opt["args"].get("init_type", "default")
            )
        )
        net.init_weights()
    return net
...
def define_metric(logger, metric_opt):
    return init_obj(
        metric_opt, logger, default_file_name="models.metric", init_type="Metric"
    )


def define_schedulers(logger, scheduler_opt):
    return init_obj(
        scheduler_opt,
        logger,
        default_file_name="models.schedulers",
        init_type="Scheduler",
    )
